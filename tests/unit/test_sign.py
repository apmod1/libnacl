...
class TestSigning(unittest.TestCase):
    ...
    def test_key_decomposition(self):
        ...
        self.assertEqual(
            prv_key, sf.crypto_sign_ed25519_sk_to_seed(secret_key))
        self.assertEqual(
            pub_key, sf.crypto_sign_ed25519_sk_to_pk(secret_key))
    ...
    def test_key_decomposition_rejects_wrong_key_lengths(self):
        ...
        """
        for test_func in (sf.crypto_sign_ed25519_sk_to_seed, sf.crypto_sign_ed25519_sk_to_pk):
    ...
    def test_sign_rejects_wrong_key_lengths(self):
        ...
        for test_func in (sf.crypto_sign, sf.crypto_sign_detached):
            for bad_key in (b'too short', b'too long' * 100):
                with self.assertRaises(ValueError) as context:
    ...
    def test_open_rejects_wrong_key_lengths(self):
        ...
        for bad_key in (b'too short', b'too long' * 100):
            with self.assertRaises(ValueError) as context:
                sf.crypto_sign_open(msg, bad_key)
            self.assertEqual(context.exception.args, ('Invalid public key',))

        with self.assertRaises(ValueError) as context:
            sf.crypto_sign_open(msg, good_key)
        self.assertEqual(context.exception.args,
    ...
    def test_verify_detached_rejects_wrong_key_lengths(self):
        ...
        for bad_key in (b'too short', b'too long' * 100):
            with self.assertRaises(ValueError) as context:
                sf.crypto_sign_verify_detached(
                    good_signature, msg, bad_key)
            self.assertEqual(context.exception.args, ('Invalid public key',))

        for bad_signature in (b'too short', b'too long' * 100):
            with self.assertRaises(ValueError) as context:
                sf.crypto_sign_verify_detached(
                    bad_signature, msg, good_key)
            self.assertEqual(context.exception.args, ('Invalid signature',))

        with self.assertRaises(ValueError) as context:
            sf.crypto_sign_verify_detached(good_signature, msg, good_key)
        self.assertEqual(context.exception.args,
                         ('Failed to validate message',))
